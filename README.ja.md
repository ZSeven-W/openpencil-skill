...
- `op` CLI コマンドリファレンス（アプリ制御、ドキュメント操作、ノード、インポート、エクスポート、テンプレート、レイアウト、ページ、変数、コード生成）
- バッチデザインDSLとサンドボックスJavaScriptモードの完全な例
- PenNodeスキーマ — インタラクティブウィジェットを含むv0.8.4の全ノードタイプ、プロパティ、フィル
- 40以上のセマンティックロールとスマートデフォルト
- デザイン原則 — タイポグラフィ、カラー、スペーシング、シャドウ
- レイアウトエンジンのルールとサイジング
- 階層型MCPワークフロー（skeleton → content → refine）
- コード生成パイプライン（plan → submit → assemble → clean）
- インポートサポート（SVG、HTMLページとライブURL、Webキャプチャスナップショット、Figma .figファイル）
- エクスポートと配信 — 画像、PDF、プレゼンテーションデッキ（PowerPoint、自己完結型HTML）
- シーンテンプレートとスタイルアセット（インポートしたDESIGN.mdガイドを含む）
